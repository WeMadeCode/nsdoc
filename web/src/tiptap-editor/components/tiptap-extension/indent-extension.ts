 ...
 */
export function parseIndentLevel(element: HTMLElement, minLevel: number, maxLevel: number, indentUnit: number): number {
 ...
 */
export function getListItemName(state: EditorState, listItemTypes: Set<string>): string | null {
...
export const Indent = Extension.create<IndentOptions>({
    ...
    return [
      {
        ...
        attributes: {
          indent: {
            ...
            parseHTML: element => {
              return parseIndentLevel(element, this.options.minLevel, this.options.maxLevel, this.options.indentUnit)
            },
        ...
        appendTransaction(transactions: readonly Transaction[], _oldState: EditorState, newState: EditorState) {
          ...
          if (index > 0) {
            ...
            if (afterIndex < afterParent.childCount) {
              targetIndent = (afterParent.child(afterIndex).attrs.indent as number) || 0
            }
          }
  ...
  addKeyboardShortcuts() {
    ...
    return {
      ...
      Enter: ({ editor }) => {
        if (
          !shouldOutdentAtCursor(editor.state, this.options.types, listItemTypesSet, {
            onlyIfEmpty: true,
          })
        ) {
          return false
