...
const Item = (props: { item: SuggestionItem; isSelected: boolean; onSelect: () => void }) => {
  ...
  return (
    <Button ref={itemRef} variant="ghost" data-active-state={isSelected ? 'on' : 'off'} onClick={onSelect}>
  ...
  const renderedItems = useMemo(() => {
    ...
    if (!showGroups) {
      items.forEach((item, index) => {
        rendered.push(
          <Item key={`item-${index}-${item.title}`} item={item} isSelected={index === selectedIndex} onSelect={() => onSelect(item)} />
        )
      })
      return rendered
    }
