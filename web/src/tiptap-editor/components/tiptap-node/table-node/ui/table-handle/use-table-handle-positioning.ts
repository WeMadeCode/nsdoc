 ...
 */
function rectFactory(orientation: Orientation, cell: DOMRect, table: DOMRect, dragging?: DraggingState): DOMRect {
  switch (orientation) {
    case 'row':
    ...
    middleware: [
      ...
      size({
        apply({ rects, elements }) {
          ...
          const refHeight =
            (orientation === 'row' ? (referencePosCell?.height ?? referencePosTable?.height) : referencePosTable?.height) ??
