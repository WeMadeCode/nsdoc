 ...
 * in the current editor state.
 */
function canAlignRowColumn({ editor, index, orientation }: { editor: Editor | null; index?: number; orientation?: Orientation }): boolean {
  if (!editor || !editor.isEditable || !isExtensionAvailable(editor, REQUIRED_EXTENSIONS)) {
    return false
  }
...
export function useTableAlignCell(config: UseTableAlignCellConfig) {
  const { editor: providedEditor, alignmentType, alignment, index, orientation, hideWhenUnavailable = false, onAligned } = config
