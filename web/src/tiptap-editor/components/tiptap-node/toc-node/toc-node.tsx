...
export function TocNodeComponent(props: NodeViewProps) {
  ...
  const { topOffset: optionTopOffset = 0, maxShowCount: optionMaxShowCount = 20, showTitle: optionShowTitle = true } = extension.options
  ...
  const normalizedDepths = useMemo<number[]>(() => normalizeHeadingDepths(visibleHeadings), [visibleHeadings, normalizeHeadingDepths])
  ...
  if (!visibleHeadings.length) {
    return (
      <NodeViewWrapper {...wrapperProps}>
        <div className="tiptap-table-of-contents-empty">Add headings to create a table of contents.</div>
      </NodeViewWrapper>
    )
  }
  ...
  return (
    <NodeViewWrapper {...wrapperProps}>
      {showTitle && <div className="tiptap-table-of-contents-title">Table of contents</div>}
      <nav aria-label="Table of contents" className="tiptap-table-of-contents-list">
