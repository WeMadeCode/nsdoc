...
export function TocSidebar({ className, maxShowCount = 20, topOffset = 0, ...props }: TocSidebarProps) {
  ...
  const handleContentClick = useCallback(
    (e: React.MouseEvent<HTMLAnchorElement>, item: TableOfContentDataItem) => {
      ...
      // Release focus from the sidebar anchor so it doesn't trap focus
  ...
  /**
   * Clear manual active id when the editor's active heading changes
   * (e.g. user clicks into a different section in the editor),
   * but only after the programmatic scroll settles.
   */
  ...
  useEffect(() => {
    ...
    // Ignore changes within 500ms of a click (from our own programmatic scroll)
    if (lastClickTime && now - lastClickTime < 500) {
      return
    }

    setManualActiveId(null)
  }, [selectionActiveId, highlightedHeading])
